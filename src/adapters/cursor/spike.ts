/**
 * Cursor Hook spike recorder — writes bounded metadata evidence for real
 * Hook payloads without copying prompt, response, or transcript bodies.
 *
 * Boundary: append-only evidence JSONL under an output directory. Used for
 * adapter validation spikes, not production capture.
 */
...
export async function recordSpikeEvent(
  ...
  // Keep runtime-shape evidence only; never copy prompt, response, or transcript bodies.
  const evidence = {
    at_ms: now(),
