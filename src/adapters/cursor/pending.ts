/**
 * Pending capture store — append-only JSONL turns and transcript folding
 * for Cursor stop events.
 *
 * Boundary: local filesystem only (pending/*.jsonl + bounded transcript
 * reads). Does not talk to Gateway or spawn the worker.
 */
...
export async function appendPendingEvents(
  ...
  // Encode the whole turn as one Buffer and issue a single O_APPEND write.
...
function parseEvent(line: string): PendingEvent | undefined {
  try {
    ...
    // Truncated or corrupt lines affect only themselves.
  }
  return undefined;
}
