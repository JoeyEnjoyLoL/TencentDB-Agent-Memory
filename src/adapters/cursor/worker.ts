/**
 * Cursor capture worker — one-shot owner that drains complete pending turns
 * to Gateway and optionally notifies session end.
 *
 * Boundary: holds the global adapter lock, starts Gateway via ctl, and
 * POSTs capture/session-end. Does not interpret Cursor Hook payloads.
 */
...
async function listPending(rootDir: string): Promise<PendingFile[]> {
  ...
  for (const name of names) {
    ...
    try {
      ...
      // Hook may still be creating the file; next scan will retry.
    }
  }
  return result;
}
...
export async function runWorker(options: WorkerOptions): Promise<void> {
  ...
  try {
    ...
    while (!compromised && !retained) {
      // Hooks may finish while Gateway starts or a batch delivers; rescan under lock until quiet.
