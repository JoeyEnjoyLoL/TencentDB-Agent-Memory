/**
 * Cursor adapter logger — append-only, redacted event lines under
 * rootDir/logs with simple size-based rotation.
 *
 * Boundary: best-effort local logging only. Failures never block Hooks
 * or the worker; sensitive field names are always redacted.
 */
...
export function createCursorLogger(
  ...
  return (event, fields = {}) => {
    try {
      ...
      // Logging must never block a Hook or worker.
    }
  };
}
